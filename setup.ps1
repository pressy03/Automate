$drive = Get-Volume | Where-Object { $_.DriveType -eq 'Removable' -and $_.FileSystemLabel -eq 'Ventoy' }

if ($drive) {
    $usbPath = "$($drive.DriveLetter):\Office\Setup32.exe"  # Replace with actual .exe name

    if (Test-Path $usbPath) {
        Start-Process -FilePath $usbPath
    } else {
        Write-Host "Executable not found at: $usbPath"
    }
} else {
    Write-Host "Ventoy USB drive not found."
}
...
$apps = 'Google.Chrome','Mozilla.Firefox','AnyDesk.AnyDesk', `
        'Adobe.Acrobat.Reader.64-bit','Oracle.JavaRuntimeEnvironment', `
        'RARLab.WinRAR';
